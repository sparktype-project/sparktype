// src/features/viewer/components/ViewHeaderContent.tsx
...
export default function ViewHeaderContent() {
  ...
  // Determine edit path - convert viewer path to editor path
  // Viewer path: /about -> Editor path: /sites/{siteId}/edit/content/about
  // Viewer path: / -> Editor path: /sites/{siteId}/edit (which opens the homepage file)
  const getEditUrl = useCallback(() => {
    const viewPath = navigation.currentPath;
    if (viewPath === '/') {
      // Homepage - let the editor resolve to the homepage file
      return `/sites/${siteId}/edit`;
    }
    // Remove leading slash from view path to create content path
    const contentPath = viewPath.substring(1);
    return `/sites/${siteId}/edit/content/${contentPath}`;
  }, [navigation.currentPath, siteId]);
