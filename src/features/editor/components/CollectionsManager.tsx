...
import { getCollections, getCollectionStats, deleteCollection } from '@/core/services/collections.service';
...
export default function CollectionsManager({ siteId }: CollectionsManagerProps) {
  ...
  const filteredCollections = useMemo(() => {
    ...
    return collections.filter(collection => 
      ...
      collection.typeId.toLowerCase().includes(filter)
    );
  ...
  const confirmDeleteCollection = async () => {
    ...
    try {
      // Delete the collection from the manifest
    ...
    return (
      <div className="flex h-full flex-col">
        ...
        <div className="p-4 text-center text-muted-foreground">
          <p className="text-sm">Loading site data...</p>
        </div>
      </div>
    );
  }
            ...
            {collections.length === 0 ? (
              <div className="space-y-3">
                <div className="text-muted-foreground">
                  ...
                  <p className="text-xs">Create a collection to organize your content</p>
                </div>
                <Button
                  size="sm"
                  onClick={() => setIsCreateDialogOpen(true)}
              ...
              <div className="text-muted-foreground">
                <p className="text-sm">No collections match your search</p>
              </div>
            )}
          </div>
        ) : (
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              This will not delete the content files, but they will no longer be 
...
function CollectionItem({ collection, siteData, onClick, onEdit, onDelete }: CollectionItemProps) {
  const stats = getCollectionStats(siteData, collection.id);
  ...
  return (
    <div className="group flex items-center justify-between rounded-md px-2 py-1.5 hover:bg-accent">
      <div 
        ...
        onClick={onClick}
        title="Click to view collection items"
      >
        ...
        <div className="text-xs text-muted-foreground">
          {stats?.itemCount || 0} items • {collection.contentPath}
        </div>
      </div>
