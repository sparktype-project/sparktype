...
export function SlashMenu({ show, availableBlocks, currentInput, onSelectBlock, inputRef }: SlashMenuProps) {
  ...
  return (
    <div 
      className="fixed bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-lg min-w-[300px] max-h-[300px] overflow-auto z-[1000] md:max-w-[90vw]"
      style={menuStyle}
    >
      <div className="px-3 py-2 border-b border-gray-300 dark:border-gray-600 text-xs font-semibold text-gray-500 dark:text-gray-400">
        Block Types
      </div>
      {filteredBlocks.map(option => (
        <button
          ...
          className="flex items-center gap-3 px-4 py-3 bg-transparent border-none w-full text-left cursor-pointer transition-colors duration-200 hover:bg-gray-100 dark:hover:bg-gray-700"
        >
          <span className="text-base w-5 text-center flex-shrink-0 text-gray-500 dark:text-gray-400">{option.icon}</span>
